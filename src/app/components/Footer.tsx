...
function NavButton({ label, path, icon, isActive }: NavButtonProps) {
  return (
    <Link to={path} className="flex-[1_0_0] flex flex-col justify-end items-center px-[4px] gap-[1px] h-[50px]">
      <div className={`flex h-[32px] items-center justify-center rounded-[100px] shrink-0 ${isActive ? 'bg-black/25 w-full' : 'w-[64px]'}`}>
...
export function Footer() {
  ...
  return (
    <div className="bg-[#1a3178] content-stretch flex items-start justify-center px-[4px] py-[6px] fixed bottom-0 left-0 right-0 z-50 w-full" data-name="Nav - M3 NAVIGATION BAR">
